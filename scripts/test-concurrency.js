const { insertReceiptAtomic, getDb } = require('./lib/db');
const { amountToIndianWords } = require('./lib/number-to-words');
...
async function runConcurrencyTest() {
  ...
  const promises = Array.from({ length: NUM_USERS }, (_, index) => {
    ...
    return new Promise((resolve, reject) => {
      // Small microtask dispatch to ensure all 15 execute in the same event loop tick
      setImmediate(() => {
        try {
          const receipt = insertReceiptAtomic({
            date: '04/09/2026',
            ac: `AC-TEST-${userNumber}`,
            contributorName: `Donor User ${userNumber}`,
            address: `Test Street ${userNumber}`,
            district: 'Indore',
            pinCode: '452001',
            ...
            paymentMode: 'UPI',
            transactionId: `UTRCONCUR${String(userNumber).padStart(4, '0')}`,
